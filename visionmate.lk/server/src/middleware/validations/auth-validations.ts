...
export const AuthValidations = {
    ...
    largeText: (key: string) => check(key)
        .isString()
        .isLength({max: 10000}),
    noPermissions: () => check('permissions')
        .not()
        .exists(),
    zip: () => check('zip')
        .isPostalCode('US')
        .withMessage('Invalid zip code!'),
    currency: (key: string = 'price') => check(key)
        .not()
        .isEmpty()
        .isNumeric(),
    objectId: (key: string = "_id") => check(key)
        .not()
        .isEmpty()
        .withMessage(`${key} cannot be empty`)
        .custom((v) => mongoose.isValidObjectId(v))
        .withMessage(`${key} is not a valid mongoDb objectID`),
    upload: (key: string = "upload") => check()
        .not()
        .isEmpty()
        .withMessage(`${key} cannot be empty`)
        .custom((v) => mongoose.isValidObjectId(v))
        .withMessage(`${key} is invalid`),
    uploads: (key: string = "uploads") => check(`${key}.*._id`)
        .not()
        .isEmpty()
